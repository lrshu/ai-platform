...
Provider 模块实现了 **Model-as-a-Service**。它将 LLM（大语言模型）、Embedding 模型与 Rerank 模型的调用逻辑与业务逻辑完全解耦。通过适配器模式，系统可以在 OpenAI、Anthropic、Ollama、Cohere 等多家厂商之间自由切换，同时为 Post-Retrieval、Retrieval 等模块提供统一的向量、文本与重排序服务。

#### 9.2 业务流程说明

1. **工厂初始化**: 根据配置（`LLM_PROVIDER`、`EMBEDDING_PROVIDER`、`RERANK_PROVIDER`），工厂类实例化对应的 Client，并缓存单例。
2. **统一参数**: 上层传入标准化的消息或文档对，Provider 负责转换为特定厂商 API 需要的格式（如 Anthropic Messages、OpenAI ChatCompletion、Cohere Rerank 请求等）。
3. **调用与重试**: 执行 API 调用，内置指数退避重试与超时控制，确保在网络抖动或速率限制下保持稳定。
4. **统一输出**: 将厂商返回的复杂 JSON 响应统一清洗为标准的 `str`、`Generator` 或 `List[float]`、`List[float] scores` 等结构，并补齐 metadata（模型名、耗时、token 计数）。
